...
const transformOutlookEmail = (outlookEmail: any): EnhancedEmail => {
  return {
    ...
    toRecipients:
      outlookEmail.toRecipients?.map((r: any) => ({
        name: r.emailAddress?.name || "",
        address: r.emailAddress?.address || "",
      })) || [],
    ccRecipients:
      outlookEmail.ccRecipients?.map((r: any) => ({
        name: r.emailAddress?.name || "",
        address: r.emailAddress?.address || "",
      })) || [],
    bccRecipients: [],
    ...
    isForward:
      outlookEmail.subject?.startsWith("Fwd:") ||
      outlookEmail.subject?.startsWith("FW:") ||
      false,
...
const getGmailEmails = async (accessToken: string) => {
  try {
    ...
    const emailPromises = listData.messages
      ?.slice(0, 20)
...
const transformGmailEmail = (gmailEmail: any): EnhancedEmail => {
  ...
  return {
    ...
    hasAttachments:
      gmailEmail.payload?.parts?.some((part: any) => part.filename) || false,
    attachments: [],
    categories: gmailEmail.labelIds || [],

    isReply:
      getHeader("Subject").startsWith("Re:") || !!getHeader("In-Reply-To"),
    isForward:
      getHeader("Subject").startsWith("Fwd:") ||
      getHeader("Subject").startsWith("FW:"),
...
const parseEmailAddresses = (addressString: string): EmailAddress[] => {
  ...
  const addresses = addressString.split(",").map((addr) => {
    const match =
      addr.trim().match(/^(.+?)\s*<(.+)>$/) || addr.trim().match(/^(.+)$/);
    if (match) {
      return {
...
export const handleOutlookCallback: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Outlook integration" });
  }
};
...
export const handleGmailCallback: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Gmail integration" });
  }
};
