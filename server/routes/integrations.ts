...
const exchangeFacebookCode = async (code: string, redirectUri: string) => {
  ...
  const response = await fetch(
    `https://graph.facebook.com/v18.0/oauth/access_token?client_id=${process.env.FACEBOOK_CLIENT_ID}&redirect_uri=${redirectUri}&client_secret=${process.env.FACEBOOK_CLIENT_SECRET}&code=${code}`,
  );
  return response.json();
};
...
export const getIntegrationStatus: RequestHandler = (req, res) => {
  const status = {
    slack: {
      ...
      totalChannels: integrationsDB.slack.reduce(
        (acc, ws) => acc + ws.channels.length,
        0,
      ),
      unreadMessages: integrationsDB.slack.reduce(
        (acc, ws) =>
    ...
    telegram: {
      ...
      totalChats: integrationsDB.telegram.reduce(
        (acc, bot) => acc + bot.chats.length,
        0,
      ),
      unreadMessages: integrationsDB.telegram.reduce(
        (acc, bot) =>
          acc + bot.chats.reduce((chAcc, ch) => chAcc + ch.unreadCount, 0),
        0,
      ),
    },
    instagram: {
      ...
      totalConversations: integrationsDB.instagram.reduce(
        (acc, ig) => acc + ig.conversations.length,
        0,
      ),
      unreadMessages: integrationsDB.instagram.reduce(
        (acc, ig) =>
          acc +
          ig.conversations.reduce(
            (convAcc, conv) => convAcc + conv.unreadCount,
            0,
          ),
        0,
      ),
    ...
    facebook: {
      ...
      totalConversations: integrationsDB.facebook.reduce(
        (acc, fb) => acc + fb.conversations.length,
        0,
      ),
      unreadMessages: integrationsDB.facebook.reduce(
        (acc, fb) =>
          acc +
          fb.conversations.reduce(
            (convAcc, conv) => convAcc + conv.unreadCount,
            0,
          ),
        0,
...
export const initiateSlackOAuth: RequestHandler = (req, res) => {
  ...
  const scopes =
    "channels:read,chat:write,users:read,groups:read,im:read,mpim:read";
...
export const handleSlackCallback: RequestHandler = async (req, res) => {
  try {
    ...
    const slackIntegration: SlackIntegration = {
      ...
      channels:
        channelsResponse.channels?.map((ch: any) => ({
          id: ch.id,
          name: ch.name,
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Slack integration" });
  }
};
...
export const updateSlackChannel: RequestHandler = (req, res) => {
  ...
  const workspace = integrationsDB.slack.find(
...
export const connectTelegramBot: RequestHandler = async (req, res) => {
  try {
    ...
    if (!validation.success) {
      return res
        .status(400)
        .json({ error: "Invalid bot token format", details: validation.error });
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Telegram integration" });
  }
};
...
export const handleInstagramCallback: RequestHandler = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Instagram integration" });
  }
};
...
export const handleFacebookCallback: RequestHandler = async (req, res) => {
  try {
    ...
    const pagesResponse = await fetch(
      `https://graph.facebook.com/v18.0/me/accounts?access_token=${tokenResponse.access_token}`,
    );
    ...
    res
      .status(500)
      .json({ error: "Internal server error during Facebook integration" });
  }
};
...
export const disconnectIntegration: RequestHandler = (req, res) => {
  ...
  switch (platform) {
    case "slack":
      integrationsDB.slack = integrationsDB.slack.filter(
        (ws) => ws.workspaceId !== id,
      );
      break;
    case "telegram":
      integrationsDB.telegram = integrationsDB.telegram.filter(
        (_, index) => index.toString() !== id,
      );
      break;
    case "instagram":
      integrationsDB.instagram = integrationsDB.instagram.filter(
        (ig) => ig.userId !== id,
      );
      break;
    case "facebook":
      integrationsDB.facebook = integrationsDB.facebook.filter(
        (fb) => fb.pageId !== id,
      );
      break;
    default:
